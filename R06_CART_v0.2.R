...
for (i in 1:length(min_criterion))
{
  for ( j in 1:length(min_split))
  {
    for ( k in 1:length(max_depth))
    {
      ...
      tmp_tree_val_prob <- 1-unlist(tmp_tree_val_response, use.names=F)[seq(1,nrow(ploan.val)*2,2)] # 각각의 범주에 속할 확률을 내어달라 (확률값)
      tmp_tree_val_rocr <- prediction(tmp_tree_val_prob, ploan.val$ploanYN)  
      
      tmp_tree_val_cm <- table(ploan.val$ploanYN, tmp_tree_val_prediction) # matrix 를 생성
